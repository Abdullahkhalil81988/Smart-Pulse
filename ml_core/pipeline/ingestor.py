...
def load_csv(filepath: str, dataset_type: str) -> pd.DataFrame:
    ...
    if dataset_type not in {"retail", "churn"}:
        raise ValueError("dataset_type must be either 'retail' or 'churn'")

    # resolve customer id column early because both dataset paths need it
