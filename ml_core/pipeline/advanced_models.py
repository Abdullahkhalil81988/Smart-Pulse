...
def train_anomaly_detector(X: np.ndarray, y: np.ndarray) -> GaussianNB:
    # gaussian nb is lightweight and fast for anomaly-vs-normal binary decisions
    model = GaussianNB()
    # fit with supervised anomaly labels where 1 means anomalous
    model.fit(X, y)
    return model


def save_anomaly_detector(model: GaussianNB, version: int = 1) -> Path:
...
def load_anomaly_detector(version: int = 1) -> GaussianNB:
